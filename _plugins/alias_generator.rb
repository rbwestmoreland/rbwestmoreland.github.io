  ...
  class AliasGenerator < Generator
    ...
    def process_posts
      @site.posts.each do |post|
		...
		generate_aliases(post.url, post.data['redirect'])
		generate_aliases(post.url, post.data['redirects'])
      end
    end
    ...
    def process_pages
      @site.pages.each do |page|
		...
		generate_aliases(page.destination('').gsub(/index\.(html|htm)$/, ''), page.data['redirect'])
		generate_aliases(page.destination('').gsub(/index\.(html|htm)$/, ''), page.data['redirects'])
      end
    end
